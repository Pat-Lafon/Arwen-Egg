...
use crate::language::{BaseType, Constant, List, Signature, TestCase, TypeSystemBounds};
    ...
    fn from(e: &Exp) -> Self {
        match &***e {
            ActualExp::CTor(name, exp) => match (name.as_ref(), &***exp) {
                ...
                ("Nil", ActualExp::Tuple(v)) if v.is_empty() => Constant::IntList(List::new()),
                ("Cons", ActualExp::Tuple(v)) if v.len() == 2 => {
                    if let (Constant::Int(n), Constant::IntList(v)) =
                        ((&v[0]).into(), (&v[1]).into())
                    {
                        Constant::IntList(v.cons(n))
                    } else {
                        panic!()
                    }
                }
